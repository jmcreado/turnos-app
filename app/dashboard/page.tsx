...
async function getServiceMetrics(
  ...
  const { data: bookings } = await supabase
    .from("bookings")
    .select("service_id, client_email, status")
    .eq("professional_id", professionalId);
...
async function getAllBookingsWithSlots(
  ...
  const { data } = await supabase
    .from("bookings")
    .select(
      "id, professional_id, slot_id, service_id, client_name, client_email, client_phone, status, payment_id, expires_at, created_at, availability_slots(start_time, end_time)"
    )
    .eq("professional_id", professionalId)
  ...
  return [...data]
    .map((b) => {
      const { availability_slots, ...rest } = b as any;
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-zinc-400">
                Recordatorios, fidelidad y promociones — próximamente
              </p>
            </div>
